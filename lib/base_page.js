...
//TO IMPROVE
Base.prototype.getElementFromListWhereTextIs = async function (listLocator, elementText) {
    let _this = this;
    return _this.driver.wait(until.elementsLocated(listLocator), 5000).then(async function (result) {
        let ex;
        await sleep(1000);
        var elementsArray = await _this.driver.findElements(listLocator);
        elementsArray.forEach(async function (element) {
